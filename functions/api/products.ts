...
export async function onRequestPost(context: { env: Env; request: Request }) {
  ...
  try {
    ...
    const result = await context.env.DB.prepare(
      `INSERT INTO products (name, category, price, original_price, rating, reviews, tag, tag_color, image, image_alt, gallery, slug, in_stock, description, blade_length, overall_length, weight, material, handle)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       RETURNING *`
    ).bind(
